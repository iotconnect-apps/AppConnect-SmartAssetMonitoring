  ...
  constructor(public productService: ProductService,
    ...
    this.activatedRoute.params.subscribe(params => {
      if (params.deviceGuid != 'add') {
        this.isEdit = true;
        this.deviceObject = {
          entityGuid: '', kitCode: '', uniqueId: '', name: '', productTypeGuid: '', deviceItems: [new shelfObj()]}
  ...
  * */
  createFormGroup() {
    this.deviceForm = new FormGroup({
      ...
      kitCode: new FormControl('', [this._notificationService.ValidatorFn, Validators.required, Validators.pattern('^[A-Za-z0-9 ]+$')]),
 ...
 * */
  ...
  addDevice() {
    ...
    if (this.deviceForm.status === "VALID" && this.shelfForm.status === "VALID") {
      ...
      this._service.checkkitCode(this.deviceForm.value.kitCode).subscribe(response => {
