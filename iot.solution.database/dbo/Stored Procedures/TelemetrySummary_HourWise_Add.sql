...
BEGIN
	...
	BEGIN TRAN		
		...
		FROM (
		SELECT D.[guid],KA.[code] AS localName, CONVERT(DATE,A.createdDate) [Date], DATEPART(HOUR,A.createdDate) [Hour], CONVERT(DECIMAL(18,2),REPLACE(attributeValue,',','')) ValueCount
